...
niche_test <- cell_info %>%
  dplyr::filter(niche %in% niche_filter) %>%
  dplyr::select(cell_prop, patient_group, niche) %>%
  group_by(niche) %>%
  nest() %>%
  mutate(k_wallis_test = map(data, function(dat) {
    broom::tidy(kruskal.test(cell_prop ~ patient_group, data = dat))
  })) %>%
  select(niche, k_wallis_test) %>%
  unnest() %>%
  ungroup() %>%
  mutate(corr_pvalue = p.adjust(p.value))
```

Defining a basic Wilcoxon two-sided test (NOT RUN)
...
cell_info %>%
  dplyr::filter(niche %in% niche_filter) %>%
  left_join(niche_test, by = "niche") %>%
  mutate(niche = ifelse(corr_pvalue <= 0.15,
                        paste0(niche,"*"), niche)) %>%
  ...
  facet_wrap(.~ niche, nrow = 4, scales = "free_y") +
