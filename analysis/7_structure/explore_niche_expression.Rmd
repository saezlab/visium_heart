...
Load meta-data from the spot integration to generate niche compositions per sample
...
niche_gex <- niche_gex[,meta_data$col_id]
```
...
std_niche_gex_cpm <- purrr::reduce(std_niche_gex_cpm, cbind)
```
...
```{r}
gex_umap <- umap(t(std_niche_gex_cpm)) %>%
  as.data.frame()
...
draw(Heatmap((scale(red_degs_ext)),
        row_names_gp = gpar(fontsize = 5),
        row_dend_side = "left",
        heatmap_legend_param = list(direction = "horizontal")),
     heatmap_legend_side = "bottom", 
