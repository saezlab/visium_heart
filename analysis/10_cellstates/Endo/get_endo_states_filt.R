...
sc_data <- loadHDF5SummarizedExperiment("./results/ct_data_filt/Endo/Endo_states_sce/")
...
pdf("./results/ct_data_filt/Endo/qc_states.pdf")
plot(qc_panel)
dev.off()
...
all_panels <- summarize_state(ct_folder, ct_alias, 
                              ...
                              n_samples_filt, 
                              background_exclude = "Endo")

# Annotate this with HCA

mi_markers <- readRDS("./results/ct_data_filt/Endo/state_genelist.rds")
...
pdf("./results/ct_data_filt/Endo/label_mapping_plt.pdf")
...
degs_ext <- read_table2("./results/ct_data_filt/Endo/state_mrks.txt")
