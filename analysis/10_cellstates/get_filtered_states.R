...
#' In this script we perform the filtering of cell states
#' 
...
cell_type <- unlist(strsplit(cell_class_excl, ","))

# Read object to subset -----------------------------------------------------
integrated_data <- readRDS(data_path)
true_ix <- !(as.character(integrated_data@meta.data[, class_label]) %in% cell_type)
...
# Add annotations
pat_anns <- readRDS("./markers/snrna_patient_anns_revisions.rds")
raw_metadata <- integrated_data@meta.data %>%
  dplyr::select(-patient) %>%
  left_join(pat_anns, by = c("orig.ident" = "sample_id"))

integrated_data@meta.data$patient <- raw_metadata$patient
