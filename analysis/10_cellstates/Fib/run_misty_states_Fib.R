...
# State descriptions 
state_list <- list("group_1" = c("Fib-state1", "Fib-state2", "Fib-state3"),
...
run_state_ppline <- function(ct, assay_label){
  ...
  misty_outs <- map(slide_files, function(slide_file){
    ...
    useful_ct_states <- state_list[[pat_group]]
    ct_states <- all_ct_states[all_ct_states %in% useful_ct_states]
    
    # Get Myeloid features
    useful_ct_states_m <- Myeloid_state_list[[pat_group]]
    ct_states_m <- all_ct_states[all_ct_states %in% useful_ct_states_m]
    ...
    view_assays <- list("main" = "cell_states",
                        ...
                        "intra_myeloid" = "cell_states",
                        "para_myeloid" = "cell_states")
    ...
    view_features <- list("main" = ct_states,
                          ...
                          "para_cts" = cts,
                          "intra_myeloid" = ct_states_m,
                          "para_myeloid" = ct_states_m) #Use all
    ...
    mistyR::plot_interaction_heatmap(misty_res_slide, "intra_myeloid", cutoff = 0)
    
    mistyR::plot_interaction_heatmap(misty_res_slide, "para_myeloid_15", cutoff = 0)
...
performance_all_misty("./results/state_structure/Fib/group_1/")
performance_all_misty("./results/state_structure/Fib/group_2/")
performance_all_misty("./results/state_structure/Fib/group_3/")
