...
sc_data <- loadHDF5SummarizedExperiment("./results/ct_data_filt/Fib/Fib_states_sce/")
...
pdf("./results/ct_data_filt/Fib/qc_states.pdf")
plot(qc_panel)
dev.off()
...
mi_markers <- readRDS("./results/ct_data_filt/Fib/state_genelist.rds")
...
pdf("./results/ct_data_filt/Fib/label_mapping_plt.pdf")

plot(label_mapping_plt)

dev.off()

# Finally check the correlation of so called marker genes:

degs_ext <- read_table2("./results/ct_data_filt/Fib/state_mrks.txt")
